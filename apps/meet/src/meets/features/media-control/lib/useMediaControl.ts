    ...
    try {
      ...
      if (cameraPub?.track) {
        ...
        nextTick(() => {
          ...
          showToast(state.value.isVideoEnabled ? "Камера включена" : "Камера выключена");
        });
      } else {
        const stream = await navigator.mediaDevices.getUserMedia({
          video: videoConstraints(),
    ...
    try {
      ...
      if (audioTrack?.track) {
        ...
        nextTick(() => {
          ...
          showToast(state.value.isAudioEnabled ? "Микрофон включён" : "Микрофон выключен");
        });
      } else {
        const storedDeviceId = getStoredAudioInputDevice();
        const constraints: MediaStreamConstraints = {
      ...
      if (isNotAllowed) {
        console.warn(
          "toggleAudio: microphone access denied or no user gesture (e.g. global shortcut without focus). Focus the app window and try again, or allow microphone in System Settings.",
        );
      } else {
