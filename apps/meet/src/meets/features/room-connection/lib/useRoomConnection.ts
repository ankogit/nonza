...
import { Room, RoomApi } from "@shared/entities";
import {
  Room as LiveKitRoom,
  RoomEvent,
  ConnectionState,
...
export interface UseRoomConnectionReturn {
  ...
  disconnect: () => void;
  updateParticipantMetadata: (payload: { color?: string | null }) => void;
...
export function useRoomConnection(roomApi: RoomApi): UseRoomConnectionReturn {
    ...
    try {
      ...
      const iceServers = tokenResponse.ice_servers?.length
        ? tokenResponse.ice_servers.map((s) => {
            ...
            const username =
              s.username ?? (s as { Username?: string }).Username;
            const credential =
              s.credential ?? (s as { Credential?: string }).Credential;
            return {
              urls,
      ...
      if (livekitRoom.localParticipant) {
        ...
        const nextNames = { ...state.value.participantNames };
        if (local.name) nextNames[local.identity] = local.name;
        const nextMap = new Map(state.value.participants);
        nextMap.set(local.identity, local);
        state.value = {
          ...state.value,
          participantNames: nextNames,
          participants: nextMap,
          participantsVersion: state.value.participantsVersion + 1,
        };
        const room = state.value.room;
        const orgColor =
          room?.current_user_org_color?.trim() || null;
        const authUser = getAuthState()?.user;
        const profileColor =
          authUser?.color && authUser.color.trim() !== ""
            ? authUser.color
            : null;
        const participantColor =
          orgColor ?? profileColor ?? DEFAULT_PARTICIPANT_COLOR;
        try {
          local.setMetadata(JSON.stringify({ color: participantColor }));
        } catch {
  ...
  const setupEventListeners = (room: LiveKitRoom) => {
    ...
    room.on(
      ...
      (participant: RemoteParticipant) => {
        ...
        showToast(`${displayName} зашёл в комнату`, { variant: "info" });
        setupParticipantListeners(participant);
    ...
    room.on(
      ...
      (participant: RemoteParticipant) => {
        ...
        showToast(`${displayName} вышел из комнаты`, { variant: "info" });
        nextTick(() => {
  ...
  const updateParticipantMetadata = (payload: { color?: string | null }) => {
    const local = state.value.livekitRoom?.localParticipant;
    if (!local) return;
    const color = payload.color ?? DEFAULT_PARTICIPANT_COLOR;
    try {
      local.setMetadata(JSON.stringify({ color }));
    } catch {
      // ignore if token does not allow metadata update
    }
  };
  ...
  const updateParticipantName = (name: string) => {
    ...
    if (!local) return;
    try {
      local.setName(name.trim());
      state.value.participantNames[local.identity] = name.trim();
      state.value.participantsVersion++;
    } catch {
      // ignore if token does not allow metadata update
    }
