  ...
  >
    <template v-if="mode === 'grid'">
      <div
        ...
        class="player-avatar"
        :class="{ 'player-avatar--accent': !!participantColor }"
        :style="
          participantColor
            ? { '--player-accent': participantColor }
            : undefined
        "
      >
        <video
        ...
        <div
          ...
          :class="{ 'player-avatar__placeholder--accent': !!participantColor }"
        >
    ...
    <div class="player-menu bg-dark-blur-90">
      <div
        class="player-menu-inner"
        :class="{ 'player-menu-inner--me': isLocalParticipant }"
      >
        <div class="left">
          <span
            v-if="mode === 'list'"
            class="player-menu__avatar"
            :style="{
              backgroundColor: participantColor || 'var(--color-primary, #2980b9)',
            }"
            aria-hidden="true"
          >
            {{ (participantName || '?').charAt(0).toUpperCase() }}
          </span>
          <div v-memo="nameReplicaMemo" class="player-menu__identity">
        ...
        <div class="right">
          <Indicator
            ...
            variant="warning"
            title="Поднята рука"
            aria-label="Поднята рука"
            class="player-menu__hand"
          >
...
watch(
  ...
  (v) => {
    if (v) {
      console.log("[Player] hasRaisedHand=true", {
        identity: (props.participant as { identity?: string })?.identity,
        participantName: props.participantName,
      });
    }
  },
...
.player-avatar__placeholder--accent {
  background: var(--player-accent, #2980b9);
}
