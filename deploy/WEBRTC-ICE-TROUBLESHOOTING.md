...
### Список команд на шлюзе (95.143.188.166)
...
sudo iptables -t nat -A PREROUTING -d 95.143.188.166 -p tcp --dport 5349 -j DNAT --to-destination 10.50.0.103:5349

# 2. DNAT: TURN relay (UDP 49152–49200) → хост с coturn
sudo iptables -t nat -A PREROUTING -i br+ -d 95.143.188.166 -p udp --dport 49152:49200 -j DNAT --to-destination 10.50.0.118
sudo iptables -t nat -A PREROUTING -d 95.143.188.166 -p udp --dport 49152:49200 -j DNAT --to-destination 10.50.0.118
sudo iptables -t nat -A OUTPUT -d 95.143.188.166 -p udp --dport 49152:49200 -j DNAT --to-destination 10.50.0.118
# SNAT: пакеты от 10.50.0.118 к relay приходят с src 95.143.188.166 — coturn примет по CREATE_PERMISSION(95.x)
sudo iptables -t nat -A POSTROUTING -s 10.50.0.118 -d 10.50.0.118 -p udp --dport 49152:49200 -j SNAT --to-source 95.143.188.166

# 3. DNAT: LiveKit RTC (UDP 50000–50100) → хост с LiveKit
sudo iptables -t nat -A PREROUTING -d 95.143.188.166 -p udp --dport 50000:50100 -j DNAT --to-destination 10.50.0.118
...
```bash
sudo iptables -t nat -D PREROUTING -d 95.143.188.166 -p udp --dport 50000:50100 -j DNAT --to-destination 10.50.0.118
sudo iptables -t nat -D POSTROUTING -s 10.50.0.118 -d 10.50.0.118 -p udp --dport 49152:49200 -j SNAT --to-source 95.143.188.166
sudo iptables -t nat -D OUTPUT -d 95.143.188.166 -p udp --dport 49152:49200 -j DNAT --to-destination 10.50.0.118
sudo iptables -t nat -D PREROUTING -d 95.143.188.166 -p udp --dport 49152:49200 -j DNAT --to-destination 10.50.0.118
sudo iptables -t nat -D PREROUTING -i br+ -d 95.143.188.166 -p udp --dport 49152:49200 -j DNAT --to-destination 10.50.0.118
sudo iptables -t nat -D PREROUTING -d 95.143.188.166 -p tcp --dport 5349 -j DNAT --to-destination 10.50.0.103:5349
sudo netfilter-persistent save
```
...
У нас: шлюз 95.143.188.166, LiveKit и coturn на 10.50.0.118, nginx на 10.50.0.103. Правила — **на шлюз**. DNAT на 10.50.0.118 и **SNAT** (источник 10.50.0.118 → 95.143.188.166): пакеты от LiveKit к 95.x:49152–49200 после DNAT уходят на хост 10.50.0.118 уже с src 95.143.188.166, Docker доставляет их в coturn — coturn видит peer 95.x. **На шлюзе используй блок команд выше** («Список команд на шлюзе») — только iptables, скрипт не нужен.
...
**Что сделать:** (1) Во фронте **не** передавать `iceTransportPolicy: "relay"` (в nonza так и сделано: политика по умолчанию, TURN только в ice_servers как запасной путь). Тогда клиент пробует host/srflx первыми; при 701 или в строгих NAT использует relay. (2) С другой сети/другого браузера (без VPN, один интерфейс) — 701 обычно не воспроизводится. (3) Не блокировать подключение при одной только ошибке 701: если есть и другие кандидаты, ICE может выбрать рабочий путь.
