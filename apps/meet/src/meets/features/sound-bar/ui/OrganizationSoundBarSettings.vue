...
function onListPointerDown(s: OrganizationSound, ev: PointerEvent) {
  ...
  ev.preventDefault();
  const existing = sessionByEmoji.value[s.emoji];
  if (existing) {
    stopSoundBarSession(existing);
    const next = { ...sessionByEmoji.value };
    delete next[s.emoji];
    sessionByEmoji.value = next;
  }
  const sessionId = createSessionId();
  sessionByEmoji.value = { ...sessionByEmoji.value, [s.emoji]: sessionId };
...
function onListPointerUp(s: OrganizationSound) {
  if (!s.gateEnabled) return;
  const sessionId = sessionByEmoji.value[s.emoji];
  if (!sessionId) return;
